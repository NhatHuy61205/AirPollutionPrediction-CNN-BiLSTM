...
def make_supervised_features(df: pd.DataFrame, cfg: LGBMBaseConfig,
    ...
    if extra_feature_cols:
        missing = [c for c in extra_feature_cols if c not in out.columns]
        if missing:
            raise ValueError(f"Missing feature cols: {missing}")

    return out

def make_multi_horizon_targets(df_feat: pd.DataFrame, cfg: LGBMBaseConfig) -> pd.DataFrame:
    out = df_feat.copy()
    for h in range(1, cfg.horizon + 1):
        out[f"y_t+{h}"] = out[cfg.target_col].shift(-h)
    return out

def build_xy(df: pd.DataFrame, cfg: LGBMBaseConfig,
...
def fit_model(X_train, y_train, cfg: LGBMBaseConfig) -> MultiOutputRegressor:
    base = LGBMRegressor(**cfg.lgbm_params)
